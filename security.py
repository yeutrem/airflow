    ...
    def get_accessible_dag_ids(
        ...
        for role in roles:
            for permission in role.permissions:
                ...
                # Defautl roles
                if {role} - {'Admin', 'Viewer', 'User', 'Op', 'Public'}:
                    dag_list_by_owner = dag_list_by_owner | self.get_dag_ids_by_role(role)

        return {dag.dag_id for dag in session.query(DagModel.dag_id).filter(DagModel.dag_id.in_(resources))} | dag_list_by_owner
    
    @provide_session
    def get_dag_ids_by_role(self, role, session=None):
        all_dags_models = session.query(DagModel.dag_id) \
            .filter(DagModel.owners.regexp_match(f"{role}[,]?|{role}$")).all()
        return set([dag.dag_id for dag in all_dags_models])
